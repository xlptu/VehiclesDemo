...
def parse_and_post(csv_file_path, post_url):
    with open(csv_file_path, newline='', encoding='utf-8') as csvfile:
        ...
        for idx, row in enumerate(reader, start=1):
            ...
            # Debug: Print the first payload
            if idx == 1:
                print("First POST payload will be:\n")
                print(json.dumps(payload, indent=4))
                print("\nStopping program after first payload for debug.")
                sys.exit(0)  # Exit the script cleanly after first print
